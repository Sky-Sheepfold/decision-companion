...
CREATE TABLE IF NOT EXISTS profile_memory_audit_log (
    ...
    action            VARCHAR(20) NOT NULL COMMENT 'confirm/reject/correct/delete/gate_reject',
    before_snapshot   JSON COMMENT '变更前快照',
    after_snapshot    JSON COMMENT '变更后快照',
    reason            VARCHAR(500) COMMENT '用户原因或系统备注',
    gate_verdict      VARCHAR(20) COMMENT 'PostureGate 门控裁判结论（accept/reject/downgrade/shadow）',
    created_at        DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
